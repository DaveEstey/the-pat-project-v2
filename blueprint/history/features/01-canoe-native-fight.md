...
throw spears, click-to-shoot can destroy a spear or its thrower, and spear hits cost
health. Three hits restarts the scene. This is the only encounter until it works.
...
- Player has **3 health**. A spear hit subtracts 1. At 0, reload the canoe scene
  with `call_deferred` (never reload inside a physics callback)
- Hitscan ignores the player hurtbox so shots can hit spears and natives
- Small health readout on the HUD (grey box, not a full UI system)
...
- Free look, driving the canoe, inventory HUD
...
- [x] **Step 6 - Health, deferred reload, reliable shooting** - Three-hit health; spear damage and scene reload run deferred; hitscan skips the player hurtbox so spears and natives are clickable; downed natives stop throwing and look darker. *Done when:* you can shoot a spear and a native; one spear does not restart; three hits restart with no physics-callback error.
...
- `health` (int) - starts at 3; each spear hit subtracts 1
...
- In-flight projectile. Queue-free if shot. Each player hit subtracts 1 health.
...
- Step 6: one spear does not restart; three hits restart with no physics error; click spear and click native both work
...
- Click is shoot only. Do not add interact, cover, or continue UI. A small health readout is in scope for Step 6.
- Reload the scene with `call_deferred`. Never `reload_current_scene()` from `area_entered`.
