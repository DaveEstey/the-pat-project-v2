...
> directory, and do not re-init the Godot project. `project.godot` is already here.
...
5. **Test** - Verify the done-when with evidence. If `AGENTS.md` declares a
   ...
   documented run-game command and the test command when configured. A step that
   adds logic must ship a passing test when the test gate is on; gameplay and
   UI steps ride on Godot play-mode, screenshot, and editor Output evidence. Run
...
13. **Release prep (optional)** - run `/release` after a completed feature or
    milestone when you want export-preset review, env var review, and a
    smoke-test path. This is a Windows Godot game, not a Render/Vercel web app;
    keep desktop export in mind. `/release` must stop before deploy, store
    upload, remote env changes, push, or publish unless the user gives a
...
- `/check` for this project means running the game in Godot (Play in the editor
  or `godot --path .`), not a web browser.
