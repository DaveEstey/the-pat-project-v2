...
The Pat Project is a game built with Godot 4 (GDScript). Target platform so far:
Windows.

This repo also uses the **AI Blueprint**, a workflow layer, not an app skeleton.
Never run a framework scaffolder inside a directory that already holds the
blueprint files (`AGENTS.md`, `CLAUDE.md`, `.agents/`, `.claude/`, `blueprint/`).
Do not re-scaffold the Godot project either; `project.godot` is already here.
...
For this Godot project, use exact engine commands, not a Node package script.
Record the exact command under Commands below when a combined Verify command
exists.
...
Godot 4. Open the folder in the Godot editor (Play / F5). The `godot` CLI is
not on PATH on this machine yet; when it is:

- Open editor: `godot --editor --path .`
- Run game: `godot --path .`
- Export / packaged build: not configured yet (`export_presets.cfg` is missing)
- Lint: not configured
